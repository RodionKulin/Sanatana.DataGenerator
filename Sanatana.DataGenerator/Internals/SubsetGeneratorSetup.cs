...
namespace Sanatana.DataGenerator.Internals
{
    public class SubsetGeneratorSetup
    {
        ...
        //init
        public SubsetGeneratorSetup(GeneratorSetup generatorSetup)
        {
            _generatorSetup = generatorSetup;
            SetupSingleTargetCount();
        }

        public GeneratorSetup ToGeneratorSetup()
        {
        ...
        public virtual object[] ReturnMulti(Type entityType)
        {
            ...
            //validate entities
            if (!services.EntityDescriptions.ContainsKey(entityType))
            {
                throw new ArgumentException($"Entity of type {entityType.Name} not found in {nameof(services.EntityDescriptions)}. Need to invoke {nameof(GeneratorSetup)}.{nameof(GeneratorSetup.RegisterEntity)} method first.");
            }
        ...
        public virtual Dictionary<Type, object[]> ReturnMulti(Type[] entityTypes)
        {
            ...
            string[] missingTypes = entityTypes
                .Where(entityType => !services.EntityDescriptions.ContainsKey(entityType))
                .Select(entityType => $"Entity of type {entityType.Name} not found in {nameof(services.EntityDescriptions)}. Need to invoke {nameof(GeneratorSetup)}.{nameof(GeneratorSetup.RegisterEntity)} method first.")
                .ToArray();
            ...
            List<InMemoryStorage>[] inMemoryStorages = entityTypes
               .Select(entityType => services.EntityDescriptions[entityType].PersistentStorages)
