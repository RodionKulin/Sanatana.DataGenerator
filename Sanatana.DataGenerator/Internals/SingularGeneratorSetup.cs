...
namespace Sanatana.DataGenerator.Internals
{
    public class SingularGeneratorSetup
    {
        ...
        {
            foreach (IEntityDescription description in _generatorSetup.EntityDescriptions.Values)
            {
        ...
        public virtual object[] ReturnMulti(Type entityType)
        {
            ...
            {
                throw new KeyNotFoundException($"Entity of type {entityType.Name} not found in {nameof(_generatorSetup.EntityDescriptions)}. Need to invoke {nameof(GeneratorSetup)}.{nameof(GeneratorSetup.RegisterEntity)} method first.");
            }
            ...
            //get InMemoryStorage
            IEntityDescription description = _generatorSetup.EntityDescriptions[entityType];
            ...
            {
                throw new NotSupportedException($"Method can only be called when entity has {nameof(InMemoryStorage)} in {nameof(IEntityDescription)}.{nameof(IEntityDescription.PersistentStorages)}.");
            }
            ...
            {
                throw new IndexOutOfRangeException($"No instances of type {entityType.Name} were found in {nameof(InMemoryStorage)}.");
            }
        ...
        public virtual Dictionary<Type, object[]> ReturnMulti(Type[] entityTypes)
        {
            ...
            {
                throw new KeyNotFoundException(string.Join(", ", missingTypes));
            }
            ...
            {
                throw new NotSupportedException(string.Join(", ", duplicateTypes));
            }
            ...
            List<InMemoryStorage>[] inMemoryStorages = entityTypes
               .Select(entityType => _generatorSetup.EntityDescriptions[entityType].PersistentStorages)
            ...
            {
                throw new NotSupportedException(string.Join(", ", missingInMemoryStorages));
            }
            ...
            {
                throw new IndexOutOfRangeException(string.Join(", ", missingInstances));
            }
